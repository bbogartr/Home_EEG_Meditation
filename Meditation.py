...
info = StreamInfo(name='Trigger',type='Markers',channel_count=1,channel_format='int32',source_id='CARE')
...
expName = 'Meditation'  # from the Builder filename that created this script
expInfo = {
...
savedatafilename='A:/Meditation_data'+ os.sep + u'data/%s_%s_%s' % (expInfo['participant'], expName, expInfo['date'])
# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='A:\\Documents\\Github\\Meditation\\Meditation.py',
    savePickle=True, saveWideText=True,
    dataFileName= savedatafilename)
# save a log file for detail verbose info
logFile = logging.LogFile(savedatafilename+'.log', level=logging.EXP)
...
# --- Initialize components for Routine "on_med" ---
...
sound_1 = sound.Sound('C', secs=.5, stereo=True, hamming=True,
    name='sound_1')
sound_1.setVolume(1.0)

# --- Initialize components for Routine "off_med" ---
sound_2 = sound.Sound('E', secs=.5, stereo=True, hamming=True,
    name='sound_2')
...
welcomeComponents = [text_test, key_resp]
...
for thisTrial in trials:
    ...
    # --- Prepare to start Routine "on_med" ---
    continueRoutine = True
    # update component parameters for each repeat
    sound_1.setSound('C', secs=1, hamming=True)
    sound_1.setVolume(1.0, log=False)
    # keep track of which components have finished
    on_medComponents = [polygon, sound_1]
    for thisComponent in on_medComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 100.0:
        ...
        if polygon.status == STARTED:
            ...
            if tThisFlipGlobal > polygon.tStartRefresh + 100.0-frameTolerance:
...
#            sound_1.status = FINISHED
    ...
    # --- Prepare to start Routine "off_med" ---
    continueRoutine = True
    # update component parameters for each repeat
    sound_2.setSound('E', secs=1, hamming=True)
    sound_2.setVolume(1.0, log=False)
    # keep track of which components have finished
    off_medComponents = [sound_2, text_rest]
    for thisComponent in off_medComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
...
#            sound_2.status = FINISHED
        defaultKeyboard.getKeys(keyList=["space"]):
            continueRoutine=False
    ...
    # --- Ending Routine "off_med" ---
    for thisComponent in off_medComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    sound_2.stop()  # ensure sound has stopped at end of routine
    outlet.push_sample(x=[20])
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
...
thisExp.saveAsWideText(savedatafilename+'.csv', delim='auto')
thisExp.saveAsPickle(savedatafilename)
logging.flush()
